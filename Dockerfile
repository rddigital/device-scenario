...
COPY --from=builder /device-scenario/cmd /
COPY --from=builder /device-scenario/LICENSE /
COPY --from=builder /device-scenario/Attribution.txt /

EXPOSE 59980

ENTRYPOINT ["/device-scenario"]
